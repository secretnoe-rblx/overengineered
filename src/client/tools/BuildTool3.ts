...
const getMouseTargetBlockPosition = (
	...
	const addTargetSize = (target: BasePart, normal: LocalVector3, pos: LocalVector3): LocalVector3 => {
		...
		if (BlockManager.isBlockPart(target)) {
			...
			globalPos = CFraming.asGlobal(target.Position);
			size = CFraming.asLocal(AABB.fromPart(target).getSize());
		}

		return pos
			.sub(
				pos
					.sub(globalPos.toLocal(plot))
					.mul(CFraming.asLocal(new Vector3(math.abs(normal.X), math.abs(normal.Y), math.abs(normal.Z)))),
			)
			.add(size.div(2).mul(normal));
	};
	const offsetBlockPivotToCenter = (selectedBlock: RegistryBlock, pos: LocalVector3) => {
		const pivot = CFraming.asGlobal(selectedBlock.model.GetPivot().Position);
		const center = CFraming.asGlobal(AABB.fromModel(selectedBlock.model).getCenter());
		const offset = CFraming.asLocal(rotation.mul(center.sub(pivot).toVector()));

		return pos.sub(offset);
	};
	const addBlockSize = (selectedBlock: RegistryBlock, normal: LocalVector3, pos: LocalVector3) => {
	...
	return targetPosition.toGlobal(plot).toVector();
};
...
namespace Scene {
	...
	export class BuildToolScene extends Control<BuildToolSceneDefinition> {
		...
		constructor(gui: BuildToolSceneDefinition, tool: BuildTool3) {
			super(gui);
			this.tool = tool;
...
namespace SinglePlaceController {
	abstract class Controller extends ClientComponent implements IController {
		protected readonly state: BuildTool3;
		...
		protected updateBlockPosition(mainPosition?: Vector3) {
			...
			if (!mainPosition) {
				...
				mainPosition = getMouseTargetBlockPosition(this.plot.get(), selectedBlock, this.blockRotation.get());
			}
			if (!mainPosition) return;
			...
			this.mainGhost.PivotTo(
				CFraming.asLocal(this.blockRotation.get()).toGlobalRotation(plot).toCFrame().add(mainPosition),
			);
	...
	export function create(tool: BuildTool3) {
...
namespace MultiPlaceController {
	...
	abstract class Starter extends ClientComponent {
		constructor(state: BuildTool3, parent: ComponentChild<IController>) {
			super();
		}
	}
	...
	export function subscribe(state: BuildTool3, parent: ComponentChild<IController>) {
	...
	export function init(
		...
		const pressPosition = getMouseTargetBlockPosition(plot, selectedBlock, state.blockRotation.get(), prevTarget);
...
const gui = ToolBase.getToolGui<"Build", Scene.BuildToolSceneDefinition>().Build.Clone();
gui.Parent = ToolBase.getToolGui<"Build", Scene.BuildToolSceneDefinition>().Build.Parent;

/** A tool for building in the world with blocks */
export class BuildTool3 extends ToolBase {
	...
	constructor(mode: BuildingMode) {
		...
		this.parentGui(new Scene.BuildToolScene(gui, this));
		...
		this.onEnable(() => {
			const rotation = CFrame.Angles(0, 45, 0);
			for (const child of mode.targetPlot.get().instance.GetChildren()) {
				if (!child.IsA("PVInstance")) continue;

				child.PivotTo(child.GetPivot().mul(rotation));
			}
		});
