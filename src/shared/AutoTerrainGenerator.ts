...
export class AutoTerrainGenerator {
	constructor(modelGenerator: TerrainModelGenerator) {
			...
			const loadChunksNextSingleRadius = (centerX: number, centerZ: number) => {
				...
				for (let num = -size; num <= size; num++) {
					for (const [x, z] of [
						...
						modelGenerator.generateTerrain(new Vector2(x, z), 0, generate(x, z));
		...
		if (false as boolean) {
			...
			task.spawn(() => {
				...
				if (false as boolean) {
					...
					modelGenerator.generateTerrain(new Vector2(0, 0), 0, generate(0, 0));
				}
			});
	}
}
